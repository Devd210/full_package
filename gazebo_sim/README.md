...
```
```
rosservice call /chief_executive/set_current_duty "duty_no: 1"
```
```
rosservice call /chief_executive/set_CEO_mode  1
```
or directly give route by (for example)
```
rosservice call /executive/set_route "file_name: 'route203.txt'"
```

Ensure that duty is present in the executive duty folder and is of correct format
Ensure that routes mentioned are present in the global_planner routes folder
