//
// Created by naivehobo on 3/29/20.
//
...
class GazeboRosSkidSteerDrive : public ModelPlugin {
 ...
 public:
  ...
  ~GazeboRosSkidSteerDrive() override {
    delete rosnode_;
  ...
  void Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf) {
    ...
    if (!_sdf->HasElement("broadcastTF")) {
      if (!this->broadcast_tf_)
        ROS_INFO_NAMED("skid_steer_drive",
                       "GazeboRosSkidSteerDrive Plugin (ns = %s) missing <broadcastTF>, defaults to false.",
                       this->robot_namespace_.c_str());
      else
        ROS_INFO_NAMED("skid_steer_drive",
                       "GazeboRosSkidSteerDrive Plugin (ns = %s) missing <broadcastTF>, defaults to true.",
                       this->robot_namespace_.c_str());
    ...
    if (!_sdf->HasElement("groundTruthTopic")) {
      ROS_WARN_NAMED("skid_steer_drive",
                     "GazeboRosSkidSteerDrive Plugin (ns = %s) missing <groundTruthTopic>, defaults to \"%s\"",
                     this->robot_namespace_.c_str(),
                     this->ground_truth_topic_.c_str());
    } else {
    ...
    if (!_sdf->HasElement("groundTruthFrame")) {
      ROS_WARN_NAMED("skid_steer_drive",
                     "GazeboRosSkidSteerDrive Plugin (ns = %s) missing <groundTruthFrame>, defaults to \"%s\"",
                     this->robot_namespace_.c_str(),
                     this->ground_truth_frame_.c_str());
    } else {
    ...
    if (!_sdf->HasElement("gaussianNoise")) {
      ROS_DEBUG_NAMED("skid_steer_drive",
                      "GazeboRosSkidSteerDrive Plugin (ns = %s) <gaussianNoise>, defaults to 0.0",
                      this->robot_namespace_.c_str());
    ...
    ros::SubscribeOptions so =
        ros::SubscribeOptions::create<geometry_msgs::Twist>(command_topic_, 1,
                                                            boost::bind(&GazeboRosSkidSteerDrive::cmdVelCallback,
                                                                        this,
                                                                        _1),
                                                            ros::VoidPtr(), &queue_);
 ...
 private:
  ...
  void publishGroundTruth(double step_time) {
    ...
    // convert velocity to child_frame_id (aka base_footprint)
    double yaw = pose.Rot().Yaw();
    ground_truth_.twist.twist.linear.x = cos(yaw) * linear.X() + sin(yaw) * linear.Y();
    ground_truth_.twist.twist.linear.y = cos(yaw) * linear.Y() - sin(yaw) * linear.X();
