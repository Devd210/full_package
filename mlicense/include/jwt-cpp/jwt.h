...
std::string extract_pubkey_from_cert(const std::string &certstr, const std::string &pw = "") {
  ...
  std::unique_ptr<BIO, decltype(&BIO_free_all)>
      certbio(BIO_new_mem_buf(certstr.data(), static_cast<int>(certstr.size())), BIO_free_all);
#endif
  std::unique_ptr<BIO, decltype(&BIO_free_all)> keybio(BIO_new(BIO_s_mem()), BIO_free_all);

  std::unique_ptr<X509, decltype(&X509_free)>
      cert(PEM_read_bio_X509(certbio.get(), nullptr, nullptr, const_cast<char *>(pw.c_str())), X509_free);
  ...
  if (!PEM_write_bio_PUBKEY(keybio.get(),
                            key.get()))
...
std::shared_ptr<EVP_PKEY> load_public_key_from_string(const std::string &key, const std::string &password = "") {
  ...
  std::shared_ptr<EVP_PKEY>
      pkey(PEM_read_bio_PUBKEY(pubkey_bio.get(), nullptr, nullptr, (void *) password.c_str()), EVP_PKEY_free);
  if (!pkey)
...
static std::unique_ptr<BIGNUM, decltype(&BN_free)> raw2bn(const std::string &raw) {
  return std::unique_ptr<BIGNUM, decltype(&BN_free)>(BN_bin2bn((const unsigned char *) raw.data(),
...
struct rsa {
  ...
  void verify(const std::string &data, const std::string &signature) const {
    ...
    auto res = EVP_VerifyFinal(ctx.get(),
                               (const unsigned char *) signature.data(),
                               static_cast<unsigned int>(signature.size()),
                               pkey.get());
    if (res != 1)
      throw signature_verification_exception(
          "evp verify final failed: " + std::to_string(res) + " " + ERR_error_string(ERR_get_error(), NULL));
  }
  /**
   * Returns the algorithm name provided to the constructor
   * \return Algorithmname
...
struct ecdsa {
  ...
  ecdsa(const std::string &public_key,
    ...
    if (!private_key.empty()) {
      ...
      pkey.reset(PEM_read_bio_ECPrivateKey(privkey_bio.get(),
                                           nullptr,
                                           nullptr,
  ...
  std::string sign(const std::string &data) const {
    ...
    std::unique_ptr<ECDSA_SIG, decltype(&ECDSA_SIG_free)>
        sig
        (ECDSA_do_sign((const unsigned char *) hash.data(), static_cast<int>(hash.size()), pkey.get()), ECDSA_SIG_free);
    if (!sig)
      throw signature_generation_exception();
#ifdef OPENSSL10
...
struct pss {
  ...
  std::string sign(const std::string &data) const {
    ...
    if (!RSA_padding_add_PKCS1_PSS_mgf1(key.get(),
                                        (unsigned char *) padded.data(),
  ...
  void verify(const std::string &data, const std::string &signature) const {
    ...
    if (!RSA_public_decrypt(static_cast<int>(signature.size()),
                            (const unsigned char *) signature.data(),
 ...
 public:
  ...
  void verify(const decoded_jwt &jwt) const {
    ...
    if (jwt.has_expires_at()) {
      auto leeway =
          claims.count("exp") == 1 ? std::chrono::system_clock::to_time_t(claims.at("exp").as_date()) : default_leeway;
    ...
    if (jwt.has_issued_at()) {
      auto leeway =
          claims.count("iat") == 1 ? std::chrono::system_clock::to_time_t(claims.at("iat").as_date()) : default_leeway;
    ...
    if (jwt.has_not_before()) {
      auto leeway =
          claims.count("nbf") == 1 ? std::chrono::system_clock::to_time_t(claims.at("nbf").as_date()) : default_leeway;
