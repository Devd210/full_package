...
Costmap2DClient::Costmap2DClient(ros::NodeHandle &param_nh,
  ...
  param_nh.param("tf_timeout", tf_timeout_, 2.0);
  ...
  ROS_INFO("[Explore Server]: Waiting for costmap to become available, topic: %s",
           costmap_topic.c_str());
  ...
  std::string tf_error;
  while (ros::ok() && !tf_listener_->canTransform(global_frame_, robot_base_frame_, ros::Time(),
                                                  ros::Duration(0.1), &tf_error)) {
    ros::spinOnce();
    if (last_error + ros::Duration(5.0) < ros::Time::now()) {
      ROS_WARN(
          "[Explore Server]: Timed out waiting for transform from %s to %s to become available "
...
void Costmap2DClient::updateFullMap(const nav_msgs::OccupancyGrid::ConstPtr &msg) {
  ...
  ROS_DEBUG("[Explore Server]: received full new map, resizing to: %d, %d", size_in_cells_x,
            size_in_cells_y);
  ...
  ROS_DEBUG("[Explore Server]: full map update, %lu values", costmap_size);
  for (size_t i = 0; i < costmap_size && i < msg->data.size(); ++i) {
    auto cell_cost = static_cast<unsigned char>(msg->data[i]);
    costmap_data[i] = cost_translation_table__[cell_cost];
  }
  ROS_DEBUG("[Explore Server]: map updated, written %lu values", costmap_size);
}

void Costmap2DClient::updatePartialMap(
    const map_msgs::OccupancyGridUpdate::ConstPtr &msg) {
  ROS_DEBUG("[Explore Server]: received partial map update");
  global_frame_ = msg->header.frame_id;

  if (msg->x < 0 || msg->y < 0) {
    ROS_ERROR("[Explore Server]: negative coordinates, invalid update. x: %d, y: %d", msg->x,
              msg->y);
    return;
  }

  auto x0 = static_cast<size_t>(msg->x);
  auto y0 = static_cast<size_t>(msg->y);
  ...
  if (xn > costmap_xn || x0 > costmap_xn || yn > costmap_yn ||
      y0 > costmap_yn) {
    ROS_WARN("[Explore Server]: received update doesn't fully fit into existing map, "
...
geometry_msgs::Pose Costmap2DClient::getRobotPose() const {
  ...
  mw_core::getRobotPose(*tf_listener_, robot_base_frame_, global_frame_, ros::Duration(tf_timeout_), robot_pose);
