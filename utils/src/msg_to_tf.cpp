...
void sendTransform(geometry_msgs::Pose const &pose, const std_msgs::Header& orig_header, std::string child_frame_id = "")
{
  std::vector<geometry_msgs::TransformStamped> transforms;
  std_msgs::Header header = orig_header;
...
void odomCallback(nav_msgs::Odometry const &odometry) {
  ...
  if (child_frame_id.empty()) child_frame_id = g_child_frame_id;
  if (g_child_frame_id.empty()) child_frame_id = "base_link";
  sendTransform(odometry.pose.pose, odometry.header, child_frame_id);
}
...
void multiCallback(topic_tools::ShapeShifter const &input) {
  ...
  ROS_ERROR_THROTTLE(1.0, "msg_to_tf received a %s message. Supported message types: nav_msgs/Odometry geometry_msgs/PoseStamped geometry_msgs/TransformStamped sensor_msgs/Imu", input.getDataType().c_str());
}
...
int main(int argc, char** argv) {
  ...
  if (subscribers == 0) {
    ROS_FATAL("Usage: rosrun msg_to_tf msg_to_tf <topic>");
    return 1;
