...
NavFn::calcPath(int n, int * st)
{
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (stc < nx || stc > ns - nx) {  // would be out of bounds
      ROS_DEBUG("[NavFn]: PathCalc Out of bounds");
      return 0;
    }
    ...
    if (npath > 2 &&
      ...
      ROS_DEBUG("[NavFn]: PathCalc oscillation detected, attempting fix.");
    ...
    if (potarr[stc] >= POT_HIGH ||
      ...
      ROS_DEBUG("[NavFn]: Path Pot fn boundary, following grid (%0.1f/%d)", potarr[stc], npath);
      ...
      ROS_DEBUG("[NavFn]: Path Pot: %0.1f  pos: %0.1f,%0.1f",
                potarr[stc], pathx[npath - 1], pathy[npath - 1]);

      if (potarr[stc] >= POT_HIGH) {
        ROS_DEBUG("[NavFn]: [PathCalc No path found, high potential");
      ...
      if (x == 0.0 && y == 0.0) {
        ROS_DEBUG("[NavFn]: PathCalc Zero gradient");
        return 0;
      }
  ...
  ROS_DEBUG("[NavFn]: PathCalc No path found, path too long");
...
NavFn::savemap(const char * fname)
{
  ...
  FILE * fp = fopen(fn, "w");
  if (!fp) {
    ROS_DEBUG("[NavFn]: Can't open file %s", fn);
    return;
  }
